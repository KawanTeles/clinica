class Toast {
  ...
  static show(message, type = 'success') {
    ...
    const bgColors = { success: '#2ecc71', error: '#e74c3c', warning: '#f1c40f', info: '#3498db' };
    toast.style.cssText = `background:${bgColors[type]||bgColors.info}; color:white; padding:12px 20px; border-radius:4px; box-shadow:0 4px 6px rgba(0,0,0,0.1); font-family:'Inter', sans-serif; font-size:14px; opacity:0; transform:translateX(100%); transition:all 0.3s ease; display:flex; align-items:center; gap:10px; min-width:250px; pointer-events:auto;`;
  ...
  static confirm(message, onConfirm) {
    ...
    dialog.innerHTML = `
      <h3 style="margin-top:0; color:var(--admin-text-main, #333); font-size:18px;">Confirmação</h3>
      <p style="color:var(--admin-text-muted, #666); margin-bottom:20px; line-height:1.5;">${message}</p>
    ...
    document.getElementById('btn-confirm-ok').addEventListener('click', () => { overlay.remove(); onConfirm(); });
  }
}
window.Toast = Toast;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // We will implement Supabase auth in ETAPA 3
    console.log('Admin core initialized. Ready for Supabase Auth integration.');
});
